...
Команда `nmcli device set ens37 managed true` :

```
nmcli device set ens37 managed true
```

Эта команда использует инструмент `nmcli` (клиент командной строки для NetworkManager) для того, чтобы включить управление сетевым интерфейсом `ens37` системой NetworkManager.
...
- `ens37`: это имя сетевого интерфейса (например, Ethernet интерфейс).
- `managed true`: означает, что интерфейс `ens37` будет управляться NetworkManager.

Таким образом, эта команда сообщает системе NetworkManager начать управлять устройством `ens37`, если оно ранее было настроено как неуправляемое.
