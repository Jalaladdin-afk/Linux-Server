...
**Включение питания**  
Система получает питание, и включаются аппаратные компоненты, такие как процессор и оперативная память.

**POST (Power-On Self-Test)**  
BIOS или UEFI проверяют оборудование на наличие ошибок (например, память, процессор, устройства хранения).

**BIOS/UEFI**  
BIOS или UEFI инициализируют систему и находят загрузочное устройство в соответствии с порядком загрузки, заданным в настройках BIOS/UEFI.

**Загрузчик (GRUB)**  
Загрузчик, обычно **GRUB**, загружает ядро операционной системы в память. В случае, если в системе несколько операционных систем, GRUB может предоставить меню для выбора.

**Ядро**  
Когда ядро загружено, оно инициализирует оборудование, монтирует начальную файловую систему (initramfs) и подготавливает систему к запуску пользовательских процессов.

**Init (systemd)**  
После загрузки ядра процесс **init** (или **systemd** на современных системах) берет на себя управление. Этот процесс запускает необходимые системные службы, такие как сеть и логирование, и подготавливает систему к взаимодействию с пользователем.

**Системные службы**  
Система запускает все системные службы, такие как сетевые службы, синхронизация времени и другие фоново работающие процессы.

**Экран входа**  
Появляется экран входа, и система ожидает ввода логина. Это может быть командная строка или графический интерфейс (например, GDM, LightDM и другие).

**Среда пользователя**  
После успешного входа в систему настраивается окружение пользователя, запускается оболочка или графическое рабочее окружение, и пользователь может начать работать с системой.





![boot process](/Module1/img/module1.1.png)
...
Команда `nmcli device set ensstep managed true` :

```
nmcli device set ensstep managed true
```

Эта команда использует инструмент `nmcli` (клиент командной строки для NetworkManager) для того, чтобы включить управление сетевым интерфейсом `ensstep` системой NetworkManager.

- `nmcli`: это команда для работы с NetworkManager через командную строку.
- `device set`: указывает, что мы изменяем настройки устройства.
- `ensstep`: это имя сетевого интерфейса (например, Ethernet интерфейс).
- `managed true`: означает, что интерфейс `ensstep` будет управляться NetworkManager.

Таким образом, эта команда сообщает системе NetworkManager начать управлять устройством `ensstep`, если оно ранее было настроено как неуправляемое.
...
Файл `/usr/lib/NetworkManager/conf.d/10-globally-managed-devices.conf` является конфигурационным файлом для NetworkManager — утилиты, используемой для управления сетевыми подключениями в Linux-системах. 

Этот файл определяет, какие устройства сети могут управляться глобально системой, например, Ethernet-адаптерами и Wi-Fi-картами. Он используется для настройки и управления устройствами, чтобы NetworkManager мог автоматически обнаружить и настроить подключение к сети на этих устройствах. 

Файл часто используется для исключения или добавления устройств в управление NetworkManager.
...
Cтандартных директорий в Linux-системах:

### **`./`** и **`../`**
- **`./`** — это текущий каталог, в котором вы находитесь. Когда вы используете команду, такую как `ls ./`, вы получаете содержимое текущей директории.
- **`../`** — это родительский каталог, на один уровень выше текущего. Если вы хотите перейти на уровень выше в иерархии каталогов, вы используете команду `cd ../`.

### **`bin/`**
Каталог **`/bin`** (сокращение от "binary") содержит исполнимые файлы (программы), которые необходимы для работы системы. Эти программы необходимы даже в случае, если система работает в режиме "одиночного пользователя" или в режиме восстановления. Программы в **`/bin`** — это базовые утилиты, такие как `ls`, `cp`, `mv`, которые используются для работы с файлами и управления системой. Эти файлы должны быть доступны всем пользователям системы.

### **`boot/`**
Каталог **`/boot`** хранит файлы, необходимые для начальной загрузки операционной системы, такие как загрузчик, ядро Linux и его конфигурационные файлы. Без этих файлов система не сможет правильно загрузиться. В этом каталоге могут находиться файлы, такие как **`vmlinuz`** (ядро) и **`grub/`** (папка с конфигурационными файлами загрузчика).

### **`cdrom/`**
Каталог **`/cdrom`** обычно используется для монтирования CD/DVD-дисков в систему. Это место для подключения и работы с оптическими дисками. Когда вы вставляете CD или DVD в привод, содержимое диска может быть доступно в этом каталоге, что позволяет работать с его файлами.

### **`dev/`**
Каталог **`/dev`** содержит специальные файлы, которые представляют устройства в системе. В Linux все устройства (жесткие диски, терминалы, устройства ввода/вывода) отображаются как файлы в этом каталоге. Например, **`/dev/sda`** может представлять первый жесткий диск, а **`/dev/tty1`** — первый терминал.

### **`etc/`**
Каталог **`/etc`** содержит все конфигурационные файлы системы и приложений. Здесь находятся файлы, которые используются для настройки операционной системы и сервисов, таких как конфигурация сети (например, **`/etc/network/interfaces`**), настройки пользователей (например, **`/etc/passwd`**), а также различные другие настройки для программ и служб.

### **`home/`**
Каталог **`/home`** предназначен для хранения личных данных пользователей системы. Каждый пользователь имеет свою собственную подкатегорию, например, **`/home/username`**, в которой хранятся его файлы, конфигурации и рабочие данные. Это позволяет пользователю хранить свои данные отдельно от системных.

### **`lib/`** и **`lib64/`**
Каталоги **`/lib`** и **`/lib64`** содержат библиотеки, которые необходимы для работы программ. Библиотеки — это наборы функций и процедур, которые могут быть использованы программами при их выполнении. **`/lib`** обычно содержит 32-битные библиотеки, в то время как **`/lib64`** хранит 64-битные библиотеки.

### **`lost+found/`**
Каталог **`/lost+found`** используется для хранения файлов, которые могут быть повреждены или утрачены из-за сбоев файловой системы. Если система потеряет данные в процессе работы с диском (например, из-за сбоя), файлы могут быть восстановлены и помещены в этот каталог.

### **`media/`**
Каталог **`/media`** используется для автоматического монтирования внешних устройств, таких как USB-накопители, CD/DVD-диски или другие внешние устройства. Когда вы подключаете внешний диск, система обычно монтирует его в **`/media`**, и вы можете сразу работать с его содержимым.

### **`mnt/`**
Каталог **`/mnt`** — это стандартное место для временного монтирования файловых систем вручную. Он используется администраторами для монтирования дисков, разделов или сетевых ресурсов, когда требуется их временное подключение.

### **`opt/`**
Каталог **`/opt`** используется для установки стороннего программного обеспечения, которое не является частью стандартной системы. Это место для хранения коммерческих приложений или дополнительных программ, которые не входят в стандартный пакет вашего дистрибутива.

### **`proc/`**
Каталог **`/proc`** — это виртуальная файловая система, которая предоставляет информацию о процессах и состоянии ядра. Здесь находятся файлы, которые содержат динамическую информацию о запущенных процессах, системе и аппаратных устройствах. Например, **`/proc/cpuinfo`** содержит информацию о процессоре, а **`/proc/meminfo`** — о состоянии памяти.

### **`root/`**
Каталог **`/root`** — это домашняя директория суперпользователя **root**. Это место, где root может хранить свои файлы и настройки. Эта директория изолирована от обычных пользователей и обычно не используется для повседневной работы.

### **`run/`**
Каталог **`/run`** хранит временные файлы и данные, которые необходимы для работы системы в реальном времени. Например, здесь могут быть хранимые PID-файлы, сокеты или другие данные, необходимые для корректной работы процессов и служб.

### **`sbin/`**
Каталог **`/sbin`** содержит системные исполнимые файлы, которые используются для администрирования системы. Эти программы, как правило, не предназначены для повседневного использования обычными пользователями, а используются суперпользователем для настройки и управления системой (например, **`fsck`**, **`reboot`**).

### **`snap/`**
Каталог **`/snap`** используется для хранения приложений, установленных через систему управления пакетами Snap. Эти приложения работают в изолированной среде, что позволяет им быть независимыми от других программ и библиотек на вашей системе.

### **`srv/`**
Каталог **`/srv`** используется для хранения данных, предоставляемых различными сервисами или приложениями. Например, веб-сервер может хранить свои файлы в **`/srv/www`**. Это место для хранения данных, которые обслуживаются различными сервисами на сервере.

### **`sys/`**
Каталог **`/sys`** — это виртуальная файловая система, которая предоставляет информацию о системных устройствах и конфигурации ядра. Она используется для настройки и мониторинга параметров ядра и аппаратных устройств, а также взаимодействия с ядром.

### **`tmp/`**
Каталог **`/tmp`** используется для хранения временных файлов, которые создаются программами во время их работы. Эти файлы могут быть удалены системой после перезагрузки или через некоторое время. Это место для хранения файлов, которые не должны сохраняться после завершения работы программы.

### **`usr/`**
Каталог **`/usr`** используется для хранения большинства программ, библиотек и документации. Это место для установки пользовательских программ, которые не являются критичными для работы системы, а также для хранения вспомогательных файлов и библиотек, таких как **`/usr/bin`** и **`/usr/lib`**.

### **`var/`**
Каталог **`/var`** используется для хранения файлов, данные которых изменяются во время работы системы. Это журналы системы, данные баз данных, почтовые сообщения и другие переменные данные. Например, журналы можно найти в **`/var/log`**, а почтовые сообщения — в **`/var/mail`**.

Каждая из этих директорий имеет важное значение в структуре Linux-системы и обеспечивает правильную работу всей операционной системы и установленных приложений.



**Crontab** — это утилита в Linux и Unix-подобных системах, предназначенная для автоматического выполнения задач по расписанию. Она позволяет пользователю запускать команды или скрипты в определённое время, а также с заданной периодичностью.

### Основы работы crontab:

Crontab состоит из таблицы задач, которая определяет, какие команды или скрипты должны быть выполнены в какой момент времени. Этот файл может быть создан и отредактирован с помощью команды `crontab`.

Когда пользователь добавляет задачу в crontab, она будет выполнена системой на основе расписания, указанного в конфигурационном файле. Важно отметить, что crontab управляется индивидуально для каждого пользователя, и у каждого пользователя может быть свой файл crontab с разными заданиями.

### Структура записи в crontab:

Каждая строка в crontab файле состоит из шести полей. Пять первых полей задают время и дату выполнения задачи, а шестое — команду, которую нужно выполнить.

Вот синтаксис строки crontab:
...
- **Первое поле (минуты)** — указывает минуту выполнения задачи. Значение может быть от 0 до 59.
- **Второе поле (часы)** — указывает час выполнения задачи. Значение может быть от 0 до 23 (где 0 — это полночь).
- **Третье поле (день месяца)** — указывает день месяца, когда задача должна быть выполнена. Значение может быть от 1 до 31.
- **Четвёртое поле (месяц)** — указывает месяц, когда задача должна быть выполнена. Значение может быть от 1 до 12.
- **Пятое поле (день недели)** — указывает день недели, когда задача должна быть выполнена. Значение может быть от 0 до 7, где 0 и 7 — это воскресенье. Остальные числа представляют дни недели (1 — понедельник, 2 — вторник и т.д.).
- **Шестое поле (команда)** — указывает команду, которую нужно выполнить. Это может быть путь к скрипту или команда, которую нужно запустить.
...
Это означает, что задача (в данном случае, выполнение скрипта `script.sh`) будет запускаться каждый день в 2:30 ночи.

### Специальные символы в crontab:

- **Звездочка (*)** — означает "каждое возможное значение". Например, если в поле минут стоит `*`, это значит, что задача будет выполняться каждую минуту.
- **Запятая (,)** — используется для указания нескольких значений. Например, `1,3,5` в поле дней месяца означает, что задача будет выполняться в 1, 3 и 5 числа месяца.
- **Тире (-)** — указывает диапазон значений. Например, `1-5` в поле дня недели означает, что задача будет выполняться с понедельника по пятницу.
- **Косая черта (/**) — используется для задания шага. Например, `*/5` в поле минут означает выполнение задачи каждые 5 минут.

Пример использования этих символов:

```
*/5 * * * * /path/to/script.sh
```

Этот пример указывает, что скрипт будет выполняться каждые 5 минут.

### Редактирование и управление crontab:

- Чтобы отредактировать файл crontab, используется команда `crontab -e`. Она откроет файл crontab в текстовом редакторе, где можно добавить или изменить задачи.
- Чтобы просмотреть текущие задачи crontab, используется команда `crontab -l`.
- Чтобы удалить все задачи из crontab, можно использовать команду `crontab -r`.
...
```
30 2 * * * rsync -az stepit stepit@192.168.218.10:/home/stepit
```

Это означает, что задача (в данном случае, выполнение `rsync -az stepit stepit@192.168.218.10:/home/stepit`) будет запускаться каждый день в 2:30 ночи.

### Примеры:

**Ежедневное выполнение задачи в определенное время:**
   
   ```
   0 5 * * * rsync -az stepit stepit@192.168.218.10:/home/stepit
   ```

**Еженедельное выполнение задачи:**
   
   ```
   0 0 * * 0 rsync -az stepit stepit@192.168.218.10:/home/stepit
   ```

**Каждые 15 минут:**

   ```
   */15 * * * * rsync -az stepit stepit@192.168.218.10:/home/stepit
   ```

**Каждый понедельник в 9:00:**

   ```
   0 9 * * 1 rsync -az stepit stepit@192.168.218.10:/home/stepit
   ```

### Логи выполнения задач:

```
0 5 * * * rsync -az stepit stepit@192.168.218.10:/home/stepit >> /var/log/myscript.log 2>&1
```
